...
pub fn update(self: *Self) void {
    const shift = raylib.IsKeyDown(raylib.KEY_LEFT_SHIFT) or
        raylib.IsKeyDown(raylib.KEY_RIGHT_SHIFT);
    for (KEYCODES) |c| {
        if (self.cursor_offset < INPUT_BUFF_LEN - 1 and raylib.IsKeyPressed(c)) {
            ...
            //     .{
            //         self.cursor_offset,
            //         self.input_buffer[0..self.cursor_offset],
            //     },
            // );
        }
    }
...
pub fn draw(self: *Self) void {
    ...
    const wider_than_view = (size.x + self.view_right_pad) - self.view_size.v[0];
...
pub fn shellTest() !void {
    ...
    const screen_size = Vec2(u32).init(screen_width, screen_height)
        .intToFloat(f32);
